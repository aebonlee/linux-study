...
## 2026-03-18 - v1.8.3 커뮤니티 메뉴 순서 정리

### 개요
커뮤니티 드롭다운 메뉴의 순서를 사용자 이용 빈도에 맞게 재정렬.
공지사항을 최상단으로 이동하고, `/community` 기본 랜딩 페이지도 공지사항으로 변경.

### 변경 전
자격증 소개 → 학습 현황 → 공지사항 → 모의고사 도장깨기 → 게시판 → 갤러리

### 변경 후
공지사항 → 자격증 소개 → 학습 현황 → 모의고사 도장깨기 → 게시판 → 갤러리

### 수정 사항
- `src/config/site.js`: 커뮤니티 드롭다운 순서 변경, 기본 path `/community/cert-intro` → `/community/announcements`
- `src/App.jsx`: `/community` 라우트 기본 컴포넌트 `CommCertIntro` → `CommAnnouncements`

### 변경 파일
| 파일 | 변경 |
|------|------|
| `src/config/site.js` | 드롭다운 순서 변경, 기본 경로 공지사항으로 변경 |
| `src/App.jsx` | `/community` 기본 라우트 `CommAnnouncements`로 변경 |
