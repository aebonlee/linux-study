...
interface ProgressContextValue {
  ...
  recordExamResult: (examId: string, score: number, total: number) => void;
  getExamResult: (examId: string) => ExamResult | null;
  resetProgress: () => void;
  syncing: boolean;
}
...
const LESSON_KEY = 'linux-study-lessons';
const EXAM_KEY = 'linux-study-exams';

function loadLocalLessons(): string[] {
  try {
...
function saveLocalLessons(lessons: string[]) {
  localStorage.setItem(LESSON_KEY, JSON.stringify(lessons));
}

function loadLocalExams(): Record<string, ExamResult> {
  try {
...
function saveLocalExams(exams: Record<string, ExamResult>) {
  localStorage.setItem(EXAM_KEY, JSON.stringify(exams));
}

export function ProgressProvider({ children }: { children: ReactNode }) {
  const { user, isAuthenticated } = useAuth();
  const [completedLessons, setCompletedLessons] = useState<string[]>(loadLocalLessons);
  const [examResults, setExamResults] = useState<Record<string, ExamResult>>(loadLocalExams);
  ...
  useEffect(() => {
    ...
    async function mergeProgress() {
      ...
      try {
        ...
        const cloudIds = (cloudLessons || []).map((r: any) => r.problem_id);
  ...
  const recordExamResult = useCallback((examId: string, score: number, total: number) => {
    setExamResults(prev => {
  ...
  const getExamResult = useCallback((examId: string) => {
