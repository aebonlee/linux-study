...
const ExamGrade1R1 = () => {
  ...
  const handleSubmit = () => { if (Object.keys(answers).length < questions.length) { alert('모든 문제에 답을 선택해주세요.'); return; } setSubmitted(true); const correctCount = Object.keys(answers).reduce((acc, idx) => acc + (answers[Number(idx)] === questions[Number(idx)].a ? 1 : 0), 0); recordExamResult('exam-grade1-r1', correctCount, questions.length); window.scrollTo({ top: 0, behavior: 'smooth' }); };
