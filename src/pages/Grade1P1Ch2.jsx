...
const Grade1P1Ch2 = () => {
  ...
  return (
    <>
      <SEOHead
        title="1급 1차 - 리눅스 시스템 관리 - Linux Study"
        description="PAM/LDAP, LVM/RAID, 커널 파라미터 튜닝, 시스템 모니터링을 학습합니다."
      />

      <section className="page-header">
        <div className="container">
          <h1 className="page-title" data-aos="fade-up">1차 - 리눅스 시스템 관리</h1>
          <p className="page-subtitle" data-aos="fade-up" data-aos-delay="100">
            사용자/그룹 관리 심화, 디스크/스토리지 관리, 커널 튜닝, 시스템 모니터링 (고급)
          </p>
        </div>
      </section>
      ...
      <div className="lesson-body container">
        ...
        <section className="lesson-section" data-aos="fade-up">
          <h2>1. 사용자/그룹 관리 심화</h2>

          <h3>PAM (Pluggable Authentication Modules)</h3>
          <p>
            PAM은 리눅스에서 인증 메커니즘을 모듈화하여 관리하는 프레임워크입니다.
            응용 프로그램은 PAM을 통해 다양한 인증 방법(패스워드, 지문, OTP 등)을 유연하게 사용할 수 있습니다.
          </p>

          <div className="code-block">
            <div className="code-header">PAM 설정 구조</div>
            <pre>{`# PAM 설정 파일 위치
...
# optional  - 선택적`}</pre>
          ...
          <div className="code-block">
            <div className="code-header">PAM을 이용한 비밀번호 정책 설정</div>
            <pre>{`# /etc/security/pwquality.conf
minlen = 8          # 최소 길이
dcredit = -1        # 최소 숫자 1개
ucredit = -1        # 최소 대문자 1개
lcredit = -1        # 최소 소문자 1개
...
@group1    soft   nproc    50     # 그룹 단위 설정`}</pre>
          </div>

          <h3>LDAP (Lightweight Directory Access Protocol)</h3>
          <p>
            LDAP는 네트워크를 통해 사용자 정보를 중앙 집중식으로 관리하는 디렉터리 서비스 프로토콜입니다.
            리눅스에서는 OpenLDAP이 대표적인 LDAP 서버 구현체입니다.
          </p>
          <div className="code-block">
            <div className="code-header">LDAP 기본 명령어</div>
            <pre>{`# OpenLDAP 서버 설치
...
# LDAP 포트: 389 (LDAP), 636 (LDAPS)`}</pre>
          </div>
        </section>
        ...
        <section className="lesson-section" data-aos="fade-up">
          ...
          <p>
            LVM은 물리 디스크를 논리적으로 관리하여, 파티션 크기를 유연하게 조절할 수 있는 기술입니다.
          </p>

          <div className="code-block">
            <div className="code-header">LVM 구조와 관리</div>
            <pre>{`# LVM 계층 구조:
...
$ vgextend data_vg /dev/sdd1`}</pre>
          ...
          <div className="lesson-table-wrapper">
            <table className="lesson-table">
              <thead>
                <tr><th>RAID 레벨</th><th>최소 디스크</th><th>특징</th><th>용량 효율</th><th>내결함성</th></tr>
              </thead>
              <tbody>
                <tr><td><strong>RAID 0</strong></td><td>2</td><td>스트라이핑 (속도 향상)</td><td>100%</td><td>없음</td></tr>
                <tr><td><strong>RAID 1</strong></td><td>2</td><td>미러링 (복제)</td><td>50%</td><td>1개 디스크 장애</td></tr>
                <tr><td><strong>RAID 5</strong></td><td>3</td><td>분산 패리티 스트라이핑</td><td>(N-1)/N</td><td>1개 디스크 장애</td></tr>
                <tr><td><strong>RAID 6</strong></td><td>4</td><td>이중 분산 패리티</td><td>(N-2)/N</td><td>2개 디스크 장애</td></tr>
                <tr><td><strong>RAID 10</strong></td><td>4</td><td>미러링 + 스트라이핑</td><td>50%</td><td>각 미러 1개씩</td></tr>
              </tbody>
            </table>
          </div>

          <div className="code-block">
            <div className="code-header">mdadm으로 소프트웨어 RAID 구성</div>
            <pre>{`# RAID 5 생성 (3개 디스크)
...
# 새 디스크로 교체
$ mdadm /dev/md0 --add /dev/sde1`}</pre>
          </div>
        </section>
        ...
        <section className="lesson-section" data-aos="fade-up">
          ...
          <div className="code-block">
            <div className="code-header">/proc 파일 시스템과 sysctl</div>
            <pre>{`# /proc 주요 파일
...
# kernel.shmmax        - 공유 메모리 최대 크기`}</pre>
          </div>
        </section>
        ...
        <section className="lesson-section" data-aos="fade-up">
          ...
          <div className="code-block">
            <div className="code-header">시스템 모니터링 도구</div>
            <pre>{`# sar - 시스템 활동 보고 (sysstat 패키지)
...
# dstat - 실시간 시스템 통계
$ dstat -cdnm 1                  # CPU, 디스크, 네트워크, 메모리`}</pre>
          </div>

          <div className="callout-box tip">
            <strong>시험 팁:</strong> Load Average에서 값이 CPU 코어 수보다 높으면 시스템이 과부하 상태입니다.
            예) 4코어 시스템에서 Load Average가 4.0이면 CPU가 100% 사용 중이라는 의미입니다.
          </div>
        </section>
        ...
        <section className="lesson-section" data-aos="fade-up">
          ...
          <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
            <Link to="/grade1/part2-ch1" className="btn btn-primary">2차 - 네트워크 및 서비스 관리 &rarr;</Link>
