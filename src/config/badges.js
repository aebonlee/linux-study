...
const BADGES = [
  ...
  },
  {
    ...
    icon: 'sword',
    color: '#FD79A8',
    condition: (completedLessons) => completedLessons.length >= 15
  },
  // Grand master - all lessons done
  {
    ...
    icon: 'trophy',
    color: '#FFD700',
    condition: (completedLessons) => completedLessons.length >= lessons.length
  }
];

export function getBadges(completedLessons) {
