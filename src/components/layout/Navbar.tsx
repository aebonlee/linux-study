...
interface MenuItem {
  ...
  path: string;
  activePath?: string;
  dropdown?: { label: string; labelKey: string; path: string }[];
}
...
const Navbar = () => {
  ...
  const menuItems: MenuItem[] = site.menuItems.map((item: any) => ({
    ...item,
    label: t(item.labelKey),
    dropdown: item.dropdown
      ? item.dropdown.map((sub: any) => ({ ...sub, label: t(sub.labelKey) }))
      : undefined
  }));
      ...
      <div className="container">
        <div className="nav-wrapper">
          <div className="logo">
            <Link to="/">
              <h1>
                {site.brand.parts.map((part: any, i: number) => (
