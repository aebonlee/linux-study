export const translations = {
  ko: {
    nav: {
      home: '홈',
      about: '소개'
    },
    site: {
      nav: {
        intro: '리눅스 개론',
        whatIsLinux: '리눅스란?',
        history: '리눅스의 역사',
        distributions: '리눅스 배포판',
        grade2: '2급 시험',
        grade2Overview: '2급 시험 개요',
        grade2P1Ch1: '1차 - 리눅스 운영 및 관리',
        grade2P1Ch2: '1차 - 리눅스 활용',
        grade2P1Ch3: '1차 - 리눅스 기초 명령어',
        grade2P2Ch1: '2차 - 리눅스 운영 및 관리',
        grade2P2Ch2: '2차 - 리눅스 활용',
        grade2P2Ch3: '2차 - 리눅스 기초 명령어',
        grade1: '1급 시험',
        grade1Overview: '1급 시험 개요',
        grade1P1Ch1: '1차 - 리눅스 실무의 이해',
        grade1P1Ch2: '1차 - 리눅스 시스템 관리',
        grade1P2Ch1: '2차 - 네트워크 및 서비스 관리',
        grade1P2Ch2: '2차 - 보안 및 시스템 최적화',
        grade1P3Ch1: '실기 - 서버 구축 실무',
        grade1P3Ch2: '실기 - 네트워크 보안 실무',
        commands: '명령어 사전',
        cmdBasic: '기본 명령어',
        cmdFile: '파일/디렉터리 관리',
        cmdProcess: '프로세스 관리',
        cmdNetwork: '네트워크 명령어',
        cmdAdmin: '시스템 관리',
        exam: '모의고사',
        examGrade2R1: '2급 1차 모의고사',
        examGrade2R2: '2급 2차 모의고사',
        examGrade1R1: '1급 1차 모의고사',
        examGrade1R2: '1급 2차 모의고사',
        references: '참고자료',
        training: '교육신청',
        progress: '학습현황',
        community: '커뮤니티',
        commCertIntro: '자격증 소개',
        commStats: '학습 현황',
        commAnnounce: '공지사항',
        commStamps: '모의고사 도장깨기',
        commBoard: '게시판',
        commGallery: '갤러리'
      },
      home: {
        heroDesc: '리눅스 마스터 2급/1급 자격증 취득을 위한 체계적인 학습 플랫폼입니다.',
        startLearning: '학습 시작하기',
        goToRef: '참고자료 보기',
        whatIsLinux: '리눅스 마스터 자격증이란?',
        linuxDescription: '리눅스 마스터 자격증은 한국정보통신진흥협회(KAIT)에서 주관하는 국가공인 자격증으로, 리눅스 운영체제에 대한 전문 지식과 실무 능력을 검증합니다. 2급은 기초~중급 수준, 1급은 고급 실무 수준을 평가합니다.',
        learningGoals: '학습 목표',
        goal1: '리눅스 운영체제의 구조와 원리를 이해한다',
        goal2: '리눅스 마스터 2급 1차/2차 시험에 합격한다',
        goal3: '리눅스 마스터 1급 시험에 도전할 수 있는 고급 역량을 갖춘다',
        goal4: '실무에서 리눅스 서버를 운영하고 관리할 수 있다',
        curriculum: '리눅스 마스터 학습 커리큘럼',
        curriculumDesc: '2급과 1급을 체계적으로 대비하는 리눅스 마스터 자격증 학습 과정입니다.',
        grade2Title: '리눅스 마스터 2급',
        grade2Desc: '기초부터 중급까지, 리눅스 운영체제의 핵심 개념과 실무 능력을 체계적으로 학습합니다.',
        grade1Title: '리눅스 마스터 1급',
        grade1Desc: '고급 실무 수준의 리눅스 시스템 관리, 네트워크 서비스, 보안 역량을 갖춥니다.',
        commandsTitle: '명령어 사전',
        commandsDesc: '리눅스 핵심 명령어를 카테고리별로 정리하여 빠르게 참고할 수 있습니다.',
        examCta: '모의고사로 실력을 점검하세요',
        examCtaDesc: '실전과 유사한 문제로 시험 대비 실력을 확인할 수 있습니다.'
      }
    },
    footer: {
      tagline: '리눅스 마스터 자격증 취득을 위한 체계적인 학습 플랫폼',
      quickLinks: '바로가기'
    },
    // Auth
    login: '로그인',
    loginTitle: '로그인',
    loginSubtitle: '학습 진도를 저장하고 수료증을 발급받으세요',
    signUp: '회원가입',
    signUpTitle: '회원가입',
    signUpSuccess: '인증 이메일을 확인해주세요. 이메일의 링크를 클릭하면 가입이 완료됩니다.',
    logout: '로그아웃',
    email: '이메일',
    password: '비밀번호',
    displayName: '이름',
    back: '뒤로',
    noAccount: '계정이 없으신가요?',
    hasAccount: '이미 계정이 있으신가요?',
    authError: '인증 중 오류가 발생했습니다',
    // Profile
    profile: '프로필',
    editName: '이름 수정',
    saveName: '저장',
    cancelEdit: '취소',
    completedLessons: '완료한 학습',
    examsTaken: '응시한 시험',
    syncing: '동기화 중...',
    // Badges
    badgesTitle: '뱃지',
    badgeEarned: '획득',
    badgeLocked: '미획득',
    badgeIntroMaster: '리눅스 입문 마스터',
    badgeIntroMasterDesc: '리눅스 개론 전체 완료',
    badgeGrade2Master: '2급 마스터',
    badgeGrade2MasterDesc: '2급 시험 학습 전체 완료',
    badgeGrade1Master: '1급 마스터',
    badgeGrade1MasterDesc: '1급 시험 학습 전체 완료',
    badgeCommandMaster: '명령어 마스터',
    badgeCommandMasterDesc: '명령어 사전 전체 완료',
    badgeFirstStep: '첫 걸음',
    badgeFirstStepDesc: '첫 번째 학습 완료',
    badgeExplorer: '탐험가',
    badgeExplorerDesc: '10개 학습 완료',
    badgeChallenger: '도전자',
    badgeChallengerDesc: '15개 학습 완료',
    badgeGrandMaster: '그랜드 마스터',
    badgeGrandMasterDesc: '모든 학습 완료',
    // Certificate
    certificateTitle: '수료증',
    certificateSubtitle: '모의고사에서 60점 이상을 달성하면 수료증을 발급받을 수 있습니다.',
    certificateLocked: '모의고사에서 60점 이상을 달성하면 수료증이 발급됩니다.',
    certTitle: '수 료 증',
    certAwardedTo: '아래의 학습자에게 수여합니다',
    certDescription: '리눅스 마스터 자격증 학습 과정을 성실히 이수하였음을 증명합니다.',
    certCompletedLessons: '완료 학습',
    certCompletionRate: '진도율',
    certBestExam: '최고 점수',
    certDate: '발급일',
    downloading: '다운로드 중...',
    downloadPDF: 'PDF 다운로드',
    downloadPNG: 'PNG 다운로드',
    // Progress
    progressTitle: '학습 현황',
    progressSubtitle: '나의 학습 진도와 성취를 확인하세요',
    totalCompleted: '완료한 학습',
    totalRemaining: '남은 학습',
    completionRate: '진도율',
    overallProgress: '전체 진도',
    myProgress: '나의 학습 진도',
    stampCollection: '스탬프',
    // Progress page sections
    certIntroTitle: '리눅스 마스터 자격증 소개',
    certIntroDesc: 'KAIT 주관 국가공인 자격증으로, 리눅스 전문 지식과 실무 능력을 검증합니다.',
    certLevel: '등급',
    certType: '자격 유형',
    certOrganizer: '주관기관',
    certRound: '차수',
    certFormat: '시험 형식',
    certPassing: '합격 기준',
    announceTitle: '공지사항',
    announceEmpty: '공지사항이 없습니다.',
    examStampTitle: '모의고사 도장깨기',
    examStampDesc: '모의고사를 응시하고 도장을 모아보세요!',
    examStampCompleted: '완료',
    examStampRemaining: '남음',
    boardTitle: '게시판',
    boardComingSoon: '게시판 기능이 곧 추가됩니다!',
    boardAuthor: '작성자',
    boardReplies: '댓글',
    galleryTitle: '갤러리',
    galleryComingSoon: '인포그래픽 갤러리가 곧 추가됩니다!',
    comingSoonBadge: '준비 중',
    // Announcements CRUD
    announceSubtitle: '시험일정, 사이트 업데이트, 학습 관련 공지를 확인하세요',
    announceNew: '공지 작성',
    announceEdit: '수정',
    announceDelete: '삭제',
    announceFormTitle: '제목',
    announceFormContent: '내용',
    announceFormTag: '태그',
    announcePinned: '고정',
    // Board CRUD
    boardSubtitle: '학습 관련 질문과 정보를 나눠보세요',
    boardWrite: '글쓰기',
    boardBack: '목록으로',
    boardEmpty: '게시글이 없습니다.',
    boardEdit: '수정',
    boardDelete: '삭제',
    boardViews: '조회수',
    boardCatAll: '전체',
    boardCatFree: '자유',
    boardCatQuestion: '질문답변',
    boardCatReview: '시험후기',
    boardCatTip: '학습팁',
    boardFormTitle: '제목을 입력하세요',
    boardFormContent: '내용을 입력하세요',
    boardFormCategory: '카테고리 선택',
    boardReplyWrite: '댓글 작성',
    boardReplyEmpty: '댓글이 없습니다.',
    boardReplyPlaceholder: '댓글을 입력하세요',
    boardReplyDelete: '삭제',
    boardWriteTitle: '글쓰기',
    boardEditTitle: '수정하기',
    // Gallery CRUD
    gallerySubtitle: '리눅스 학습에 도움이 되는 자료 모음',
    galleryAdd: '등록',
    galleryEdit: '수정',
    galleryDelete: '삭제',
    galleryEmpty: '갤러리 항목이 없습니다.',
    galleryFormTitle: '제목',
    galleryFormDesc: '설명',
    galleryFormImage: '이미지 URL',
    galleryFormCategory: '카테고리 선택',
    galleryCatAll: '전체',
    galleryCatProject: '프로젝트',
    galleryCatInfographic: '인포그래픽',
    galleryCatScreenshot: '스크린샷',
    galleryCatOther: '기타',
    galleryAddTitle: '갤러리 등록',
    galleryEditTitle: '갤러리 수정',
    // Common CRUD
    save: '저장',
    cancel: '취소',
    loadingText: '로딩 중...',
    loginRequired: '로그인 후 이용할 수 있습니다.',
    confirmDelete: '정말 삭제하시겠습니까?',
    // Lesson complete
    markComplete: '학습 완료',
    lessonCompleted: '학습 완료됨'
  },
  en: {
    nav: {
      home: 'Home',
      about: 'About',
    },
    site: {
      nav: {
        intro: 'Linux Intro',
        whatIsLinux: 'What is Linux?',
        history: 'History of Linux',
        distributions: 'Linux Distributions',
        grade2: 'Grade 2 Exam',
        grade2Overview: 'Grade 2 Overview',
        grade2P1Ch1: 'Round 1 - Linux Admin',
        grade2P1Ch2: 'Round 1 - Linux Usage',
        grade2P1Ch3: 'Round 1 - Basic Commands',
        grade2P2Ch1: 'Round 2 - Linux Admin',
        grade2P2Ch2: 'Round 2 - Linux Usage',
        grade2P2Ch3: 'Round 2 - Basic Commands',
        grade1: 'Grade 1 Exam',
        grade1Overview: 'Grade 1 Overview',
        grade1P1Ch1: 'Round 1 - Linux Concepts',
        grade1P1Ch2: 'Round 1 - System Admin',
        grade1P2Ch1: 'Round 2 - Network & Services',
        grade1P2Ch2: 'Round 2 - Security & Optimization',
        grade1P3Ch1: 'Practical - Server Setup',
        grade1P3Ch2: 'Practical - Network Security',
        commands: 'Command Reference',
        cmdBasic: 'Basic Commands',
        cmdFile: 'File Management',
        cmdProcess: 'Process Management',
        cmdNetwork: 'Network Commands',
        cmdAdmin: 'System Admin',
        exam: 'Mock Exam',
        examGrade2R1: 'Grade 2 Round 1',
        examGrade2R2: 'Grade 2 Round 2',
        examGrade1R1: 'Grade 1 Round 1',
        examGrade1R2: 'Grade 1 Round 2',
        references: 'References',
        training: 'Apply',
        progress: 'Progress',
        community: 'Community',
        commCertIntro: 'Certification',
        commStats: 'Stats',
        commAnnounce: 'Announcements',
        commStamps: 'Stamp Rally',
        commBoard: 'Board',
        commGallery: 'Gallery'
      },
      home: {
        heroDesc: 'A systematic learning platform for Linux Master Grade 2/1 certification.',
        startLearning: 'Start Learning',
        goToRef: 'References',
        whatIsLinux: 'What is Linux Master Certification?',
        linuxDescription: 'Linux Master Certification is a nationally recognized qualification administered by KAIT, validating professional knowledge and practical skills in Linux operating systems.',
        learningGoals: 'Learning Goals',
        goal1: 'Understand the structure and principles of Linux OS',
        goal2: 'Pass Linux Master Grade 2 examinations',
        goal3: 'Develop advanced skills for Grade 1 certification',
        goal4: 'Operate and manage Linux servers in production',
        curriculum: 'Linux Master Curriculum',
        curriculumDesc: 'A systematic certification course covering Grade 2 and Grade 1.',
        grade2Title: 'Linux Master Grade 2',
        grade2Desc: 'Systematically learn the core concepts and practical skills of Linux OS from basics to intermediate level.',
        grade1Title: 'Linux Master Grade 1',
        grade1Desc: 'Develop advanced skills in Linux system administration, network services, and security.',
        commandsTitle: 'Command Reference',
        commandsDesc: 'Quickly look up essential Linux commands organized by category.',
        examCta: 'Test Your Skills with Mock Exams',
        examCtaDesc: 'Practice with exam-style questions to prepare for the real test.'
      }
    },
    footer: {
      tagline: 'A systematic learning platform for Linux Master certification',
      quickLinks: 'Quick Links'
    },
    // Auth
    login: 'Login',
    loginTitle: 'Login',
    loginSubtitle: 'Save your progress and earn certificates',
    signUp: 'Sign Up',
    signUpTitle: 'Sign Up',
    signUpSuccess: 'Please check your email. Click the link to complete registration.',
    logout: 'Logout',
    email: 'Email',
    password: 'Password',
    displayName: 'Name',
    back: 'Back',
    noAccount: "Don't have an account?",
    hasAccount: 'Already have an account?',
    authError: 'An error occurred during authentication',
    // Profile
    profile: 'Profile',
    editName: 'Edit Name',
    saveName: 'Save',
    cancelEdit: 'Cancel',
    completedLessons: 'Completed Lessons',
    examsTaken: 'Exams Taken',
    syncing: 'Syncing...',
    // Badges
    badgesTitle: 'Badges',
    badgeEarned: 'Earned',
    badgeLocked: 'Locked',
    badgeIntroMaster: 'Linux Intro Master',
    badgeIntroMasterDesc: 'Complete all Linux Intro lessons',
    badgeGrade2Master: 'Grade 2 Master',
    badgeGrade2MasterDesc: 'Complete all Grade 2 lessons',
    badgeGrade1Master: 'Grade 1 Master',
    badgeGrade1MasterDesc: 'Complete all Grade 1 lessons',
    badgeCommandMaster: 'Command Master',
    badgeCommandMasterDesc: 'Complete all Command Reference lessons',
    badgeFirstStep: 'First Step',
    badgeFirstStepDesc: 'Complete your first lesson',
    badgeExplorer: 'Explorer',
    badgeExplorerDesc: 'Complete 10 lessons',
    badgeChallenger: 'Challenger',
    badgeChallengerDesc: 'Complete 15 lessons',
    badgeGrandMaster: 'Grand Master',
    badgeGrandMasterDesc: 'Complete all lessons',
    // Certificate
    certificateTitle: 'Certificate',
    certificateSubtitle: 'Earn a certificate by scoring 60% or above on a mock exam.',
    certificateLocked: 'Score 60% or above on a mock exam to unlock your certificate.',
    certTitle: 'Certificate of Completion',
    certAwardedTo: 'This certificate is awarded to',
    certDescription: 'For successfully completing the Linux Master certification study course.',
    certCompletedLessons: 'Lessons',
    certCompletionRate: 'Completion',
    certBestExam: 'Best Score',
    certDate: 'Date',
    downloading: 'Downloading...',
    downloadPDF: 'Download PDF',
    downloadPNG: 'Download PNG',
    // Progress
    progressTitle: 'Learning Progress',
    progressSubtitle: 'Track your study progress and achievements',
    totalCompleted: 'Completed',
    totalRemaining: 'Remaining',
    completionRate: 'Completion Rate',
    overallProgress: 'Overall Progress',
    myProgress: 'My Learning Progress',
    stampCollection: 'Stamps',
    // Progress page sections
    certIntroTitle: 'Linux Master Certification',
    certIntroDesc: 'A nationally recognized qualification by KAIT, validating Linux expertise and practical skills.',
    certLevel: 'Level',
    certType: 'Type',
    certOrganizer: 'Organizer',
    certRound: 'Round',
    certFormat: 'Format',
    certPassing: 'Passing Criteria',
    announceTitle: 'Announcements',
    announceEmpty: 'No announcements.',
    examStampTitle: 'Mock Exam Stamp Rally',
    examStampDesc: 'Take mock exams and collect stamps!',
    examStampCompleted: 'Done',
    examStampRemaining: 'Left',
    boardTitle: 'Community Board',
    boardComingSoon: 'Community board coming soon!',
    boardAuthor: 'Author',
    boardReplies: 'Replies',
    galleryTitle: 'Gallery',
    galleryComingSoon: 'Infographic gallery coming soon!',
    comingSoonBadge: 'Coming Soon',
    // Announcements CRUD
    announceSubtitle: 'Check exam schedules, site updates, and learning announcements',
    announceNew: 'New Announcement',
    announceEdit: 'Edit',
    announceDelete: 'Delete',
    announceFormTitle: 'Title',
    announceFormContent: 'Content',
    announceFormTag: 'Tag',
    announcePinned: 'Pinned',
    // Board CRUD
    boardSubtitle: 'Share questions and tips about learning',
    boardWrite: 'Write Post',
    boardBack: 'Back to List',
    boardEmpty: 'No posts yet.',
    boardEdit: 'Edit',
    boardDelete: 'Delete',
    boardViews: 'Views',
    boardCatAll: 'All',
    boardCatFree: 'Free',
    boardCatQuestion: 'Q&A',
    boardCatReview: 'Review',
    boardCatTip: 'Tips',
    boardFormTitle: 'Enter title',
    boardFormContent: 'Enter content',
    boardFormCategory: 'Select Category',
    boardReplyWrite: 'Submit',
    boardReplyEmpty: 'No replies yet.',
    boardReplyPlaceholder: 'Write a reply...',
    boardReplyDelete: 'Delete',
    boardWriteTitle: 'Write Post',
    boardEditTitle: 'Edit Post',
    // Gallery CRUD
    gallerySubtitle: 'A collection of resources for Linux learning',
    galleryAdd: 'Add',
    galleryEdit: 'Edit',
    galleryDelete: 'Delete',
    galleryEmpty: 'No gallery items.',
    galleryFormTitle: 'Title',
    galleryFormDesc: 'Description',
    galleryFormImage: 'Image URL',
    galleryFormCategory: 'Select Category',
    galleryCatAll: 'All',
    galleryCatProject: 'Project',
    galleryCatInfographic: 'Infographic',
    galleryCatScreenshot: 'Screenshot',
    galleryCatOther: 'Other',
    galleryAddTitle: 'Add Gallery Item',
    galleryEditTitle: 'Edit Gallery Item',
    // Common CRUD
    save: 'Save',
    cancel: 'Cancel',
    loadingText: 'Loading...',
    loginRequired: 'Please log in to continue.',
    confirmDelete: 'Are you sure you want to delete?',
    // Lesson complete
    markComplete: 'Mark Complete',
    lessonCompleted: 'Completed'
  }
};
